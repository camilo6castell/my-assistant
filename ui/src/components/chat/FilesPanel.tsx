...
export function FilesPanel({
  ...
  files: ReturnType<typeof useEphemeralFiles>
}) {
  ...
  return (
    <Popover.Root>
      <Popover.Trigger
        render={
          <Button variant="ghost" size="icon" aria-label="Archivos de la conversación">
      ...
      <Popover.Portal>
        <Popover.Positioner sideOffset={8} align="start">
          <Popover.Popup className="w-80 rounded-xl border border-white/10 bg-popover/95 p-4 text-sm text-popover-foreground shadow-2xl backdrop-blur-2xl">
            ...
            <div className="space-y-3">
              <label className="flex items-center justify-between">
                <span className="text-xs text-muted-foreground">
                  Adjuntar a colección permanente
                </span>
              ...
              <Button
                ...
                disabled={
                  files.upload.isPending || (attachToCollection && !collectionName.trim())
                }
                onClick={handlePickFile}
              >
                <UploadCloud className="size-3.5" />
                {files.upload.isPending ? "Subiendo..." : "Subir archivo (.pdf, .html, .txt)"}
              </Button>

              {files.upload.isError && (
                <p className="text-xs text-destructive">
                  {apiErrorMessage(files.upload.error)}
                </p>
              )}
