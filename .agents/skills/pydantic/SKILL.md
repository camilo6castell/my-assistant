...
tree = TreeNode(
    value=1, children=[TreeNode(value=2, children=[TreeNode(value=4)]), TreeNode(value=3)]
)
...
class StrictUser(BaseModel):
    ...
    def validate_age(cls, v: int) -> int:
        if v < 18:
            raise PydanticCustomError(
                "underage", "User must be at least 18 years old", {"age": v, "minimum_age": 18}
            )
        return v
...
def test_nested_validation():
    company = Company(name="ACME", address={"street": "123 Main", "city": "NYC", "country": "USA"})
...
@given(id=st.integers(min_value=1), name=st.text(min_size=1, max_size=100), email=st.emails())
