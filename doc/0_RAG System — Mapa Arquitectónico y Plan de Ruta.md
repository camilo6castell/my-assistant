...
Cómo el sistema convierte texto plano en vectores numéricos. Qué es un embedding, por qué dividimos en chunks, qué significa `normalize_embeddings=True`.

### Módulo 2 — Almacenamiento Vectorial

`src/context/storage.py` · `src/ingest/core.py` (funciones de persistencia)

Por qué guardamos tres archivos (`index.faiss`, `metadata.pkl`, `vectors.npy`), qué rol cumple cada uno, y cómo FAISS hace la búsqueda.

### Módulo 3 — Gestión de Contextos

`src/context/manager.py` · `src/context/selector.py` · `src/context/models.py`

Cómo el sistema organiza múltiples colecciones independientes y las carga/descarga de memoria.
